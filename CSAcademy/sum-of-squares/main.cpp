...
const int nsz = int(1e4);
int n, m;
rem f[2][nsz + 5][2], sum[2][nsz + 5][2], ans;

int main() {
	scanf("%d%d", &m, &n);
	int o = 0, x = 1;
	f[o][0][0] = 1, sum[o][0][0] = 0;
	cont (i, n) {
		circ (j, 0, m) {
			rem val = sum[o][j][0] + sum[o][j][1], way = f[o][j][0] + f[o][j][1];
			if (i == n && j == m) ans += sum[o][j][1];
			f[x][j][0] += way, sum[x][j][0] += val;
			if (j + i <= m) f[o][j + i][1] += way, sum[o][j + i][1] += val + way * rem(2 * j + i);
		}
		swap(o, x);
